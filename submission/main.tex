...
The accompanying code is a pip-installable package (\texttt{lieideal-hs}) that constructs Pauli operators, Trotter--Suzuki steps, exact propagators, residual generators, spectral truncations, the symmetric and first-order compilations, the certificate-driven bandwidth selection, the learned per-site model, and every table and figure from first principles. Installing it and running a single console entry point (\texttt{lieideal-reproduce}) deterministically regenerates every dataset, table, and figure of this manuscript; a companion entry point (\texttt{lieideal-verify}) reruns the pipeline and asserts byte-identical outputs. The spectral-truncation convergence rate, the certificate, and the symmetric-versus-first-order compilation are produced by the \texttt{spectral\_truncation} module. The code is intended as a reproducibility baseline and as a starting point for GPU-accelerated implementations.
