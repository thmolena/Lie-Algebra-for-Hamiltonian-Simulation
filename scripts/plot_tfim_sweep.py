...
def main() -> None:
    ...
    input_path = Path("results") / "tfim_sweep_results.csv"
    ...
    with input_path.open() as handle:
        ...
        for row in reader:
            key = (float(row["coupling_j"]), float(row["field_h"]), float(row["total_time"]))
            grouped[key].append(row)

    for (coupling_j, field_h, total_time), rows in grouped.items():
        ...
        plt.title(f"TFIM error vs gate proxy: J={coupling_j}, h={field_h}, T={total_time}")
        plt.legend()
        plt.tight_layout()

        file_name = f"tfim_J{_fmt(coupling_j)}_h{_fmt(field_h)}_T{_fmt(total_time)}.png"
