...
Three properties distinguish this from incremental improvement:
\begin{enumerate}
...
expressivity theorem, generator selection rule, locality proposition,
and multi-term extension.
...
long-time benchmarks ($T\in[0,10]$).
...
\end{equation}
This controls both gate overhead and approximation quality.
Standard commutator-scaled Trotter
analysis~\cite{childs2021,layden2022} shows that the
omitted generators contribute at most
$N\cdot\tau\cdot\delta t^2$ to the global error,
so choosing $\tau = \varepsilon/(N\delta t^2)$ bounds
the truncation error to $\varepsilon$.
