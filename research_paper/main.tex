...
To establish that the $10\times$ improvement is not restricted to
short times, we sweep total evolution time from $T=0.1$ to $T=10$
with fixed step size $\delta t$.

Figure~\ref{fig:longtime} summarizes two regimes:
\begin{itemize}
  \item $\delta t=0.04$: ratio $\geq10\times$ for $T\leq2.2$ (GPT-1 vs.\ Trotter-1)
    and $\geq10\times$ for all $T$ (GPT-2 vs.\ Suzuki-2).
  \item $\delta t=0.01$ ($N=T/\delta t\leq1000$, CPU-feasible):
    GPT-1/Trotter-1 ratio $\geq10.5\times$ for all $T\leq10$;
    GPT-2/Suzuki-2 ratio $\geq22\times$ for all $T\leq10$.
\end{itemize}

Figure~\ref{fig:longtime-nsweep} shows the $N$-sweep at $T=10$.
At $N=1000$ ($\delta t=0.01$), GPT-1 achieves $10.5\times$ and
GPT-2 achieves $169.8\times$ improvement.
The $N^{-2}$ and $N^{-3}$ reference lines confirm the theoretical
scaling (Corollary~\ref{cor:global} and the GPT-2 analog).

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{liegpt_longtime}
  \caption{Spectral error and improvement ratio vs.\ total time $T$
    for TFIM $n{=}4$, $J{=}1$, $h{=}0.5$.
    \textbf{(a,b)}~$\delta t{=}0.04$: GPT-1 achieves $\geq\!10\times$
    for $T\leq2.2$; GPT-2 achieves $\geq\!10\times$ throughout.
    \textbf{(c,d)}~$\delta t{=}0.01$: both GPT-1 ($\geq\!10.5\times$)
    and GPT-2 ($\geq\!22\times$) maintain improvement for all $T\leq10$
    with $N\leq1000$ steps (CPU-feasible).}
  \label{fig:longtime}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{liegpt_longtime_nsweep}
  \caption{$N$-sweep at $T=10$, TFIM $n{=}4$, $J{=}1$, $h{=}0.5$.
    \textbf{(a)}~Global spectral error confirms $N^{-2}$ (GPT-1) and
    $N^{-3}$ (GPT-2) scaling.
    \textbf{(b)}~Improvement ratio crosses $10\times$ at $N{=}1000$
    (GPT-1 vs.\ Trotter-1) and exceeds $\!169\times$ at $N{=}1000$
    (GPT-2 vs.\ Suzuki-2); vertical dotted line marks $N{=}1000$.}
  \label{fig:longtime-nsweep}
\end{figure}
